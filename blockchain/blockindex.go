...
const (
	indexCacheLimit = 102400 
)

func initBlockNode(cfg *types.CalorieConfig, node *blockNode, block *types.Block, broadcast bool, pid string, sequence int64) {
	*node = blockNode{
		hash:       block.Hash(cfg),
...
func newBlockNode(cfg *types.CalorieConfig, broadcast bool, block *types.Block, pid string, sequence int64) *blockNode {
	var node blockNode
